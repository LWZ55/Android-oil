...
public class Unit extends View
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        // Draw half a tick 画竖线
        canvas.drawLine(width, 0, width, height / 3, paint);

        // Set up paint
        paint.setAntiAlias(true);
        paint.setTextSize(height * 2 / 3);
        paint.setTextAlign(Align.CENTER);//设置legend位置

        // Draw half of the units 通过scale的大小控制不同的单位标度
        if (scale == 0)
        {
            canvas.drawLine(width / 3, 0, width, 0, paint); //画横线
