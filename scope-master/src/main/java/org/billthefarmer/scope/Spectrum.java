...
public class Spectrum extends View
{
    ...
    protected void onDraw(Canvas canvas)
    {
        ...
        // Calculate x scale
        float xscale = (float)Math.log(udpserver.xa.length) / width;
                ...
                graticule.getHeight() != height)
        {
            ...
            for (float m : ma)
            {
                for (float f : fa)
                {
                    float x = (float) Math.log((f * m) / udpserver.fps) / xscale;
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int i = last; i <= index; i++)
            {
                ...
                if (i > 0 && i < udpserver.xa.length)
                {
                    if (value < udpserver.xa[i])
                        value = (float)udpserver.xa[i];
                }
            }
        ...
        if (udpserver.frequency > 0.0)
        {
            // Yellow pen for frequency trace
            paint.setColor(Color.YELLOW);

            // Create line for frequency
            float x = (float)Math.log(udpserver.frequency / udpserver.fps) / xscale;
            paint.setAntiAlias(false);
